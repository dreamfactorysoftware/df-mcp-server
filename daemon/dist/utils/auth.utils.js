 ...
 * API keys are 64-character hex strings (sha256 output — see
 * App::generateApiKey in df-core). The PHP proxy enforces this format on
 * client-supplied keys before forwarding; this daemon-side check is available
 * for strict mode and for defense in depth.
 */
 ...
 */
export function getAuthHeaderFromRequest(req, headerName) {
 ...
 */
export function isValidSessionTokenFormat(sessionToken) {
    if (!sessionToken)
        return false;
 ...
 */
export function validateAuthCredentials(credentials, strictFormat = false) {
 ...
 */
export function extractAndValidateAuth(req, strictFormat = false) {
 ...
 */
export function getAuthModeDescription(mode) {
    switch (mode) {
        case 'both':
