...
// ---------------------------------------------------------------------------
const requestResponse = new AsyncLocalStorage();
export const runWithResponse = (res, fn) => requestResponse.run(res, fn);
// ---------------------------------------------------------------------------
...
export const isReadOnly = (name) => READ_ONLY.test(name);
// ---------------------------------------------------------------------------
// Per-server state
// ---------------------------------------------------------------------------
export class LazyState {
    ...
    ledger(extra) {
        ...
        if (!res || res.headersSent || !this.fullList)
            return;
...
export function installLazyFacade(server, state) {
    const facade = (name, title, description, schema, handler) => {
        ...
        server.registerTool(name, { title, description, inputSchema: schema }, async (params, ctx) => {
            try {
