...
export function registerGlobalTools(server, sessionManager, disabledTools) {
    const registerTool = createToolRegistrar(server, disabledTools);
    registerTool('discover_services', 'Discover Services', 'Discover which DreamFactory services and operations YOU (the calling agent) are '
        ...
        try {
            ...
            return respondError(`discover_services failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    registerTool('request_access', 'Request Access', 'Request access you do not currently have. Creates a pending request for human '
        ...
        + '— an administrator must approve it, after which a fresh scoped key is issued.', z.object({
        ...
        try {
            ...
            const auth = getAuth(sessionManager, context.sessionId);
            return respond(await dfFetch('POST', `${cfg.url}/agent/request_access`, auth, {
            ...
            return respondError(`request_access failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
