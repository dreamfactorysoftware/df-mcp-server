...
export function registerDreamFactoryTools(server, sessionManager) {
    const getSessionConfig = (sessionId) => {
        ...
        if (!url || (!apiKey && !sessionToken)) {
            throw new Error('DreamFactory configuration not found. Please ensure DREAMFACTORY_URL and DREAMFACTORY_API_KEY are configured either in the session or process environment.');
        }
        return {
            url,
