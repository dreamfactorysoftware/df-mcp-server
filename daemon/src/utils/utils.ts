...
export function createServer(
  ...
  const instructions = [
    ...
    '4. `aggregate_data` - Compute SUM/COUNT/AVG/MIN/MAX in ONE call (no manual pagination needed)',
    '5. `get_table_schema` - Full schema for a single table (if you need more detail)',
    '6. `create_records` / `update_records` / `delete_records` - CRUD operations',
    '7. `get_stored_procedures` / `call_stored_procedure` - Stored procedure access',
    '8. `get_stored_functions` / `call_stored_function` - Stored function access',
    '',
