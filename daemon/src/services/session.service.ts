export type SessionConfig = {
  url: string;
  sessionToken?: string; // DF JWT for user authentication (optional for API-key-only auth)
  apiKey?: string; // DF API key (required for non-admin users, can be used alone if app has role)
};
...
export class SessionService {
   ...
   */
  getStats(): { total: number; oldest: number | null; avgAge: number } {
    const now = Date.now();
