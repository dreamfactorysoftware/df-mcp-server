...
/**
 * A verb that can be registered once per service (prefixed mode) or once for
 ...
 * a one-argument call.
 */
export function registerMergedTools(
  server: McpServer,
  sessionManager: SessionService,
  configs: ApiConfig[],
  tools: MergedToolDefinition[],
  opts: { targetNoun: string; logLabel: string; disabledTools?: Set<string> }
) {
  if (configs.length === 0) {
    return;
  }
  ...
  if (notes.length > 0) {
    console.log(
      `[${logLabel}] per-service disables enforced at call time; allowed services per verb: ` +
      notes.join('; ')
    );
  }
}
