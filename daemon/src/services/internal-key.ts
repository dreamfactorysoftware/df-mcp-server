...
export function keyMatches(expected: string, presented: unknown): boolean {
 ...
 */
export function authorizeInternalCall(presented: unknown): boolean {
...
export function internalKeyGate(req: Request, res: Response, next: NextFunction): void {
...
function warnNoKey(detail: string): void {
  ...
  warned = true;
  console.error(
    `[mcp-daemon] Shared secret unavailable (${detail}). Every /mcp request is ` +
