...
// Extended AuthInfo with DreamFactory-specific fields
export interface DFAuthInfo extends AuthInfo {
  extra: {
    userId: number;
    email: string;
    dfJwt: string;
...
export class DreamFactoryOAuthProvider {
  ...
  private refreshTokens = new Map<string, { accessToken: string; expiresAt: number }>();
   ...
   */
  authorize(serviceName: string, params: AuthorizeParams): string {
    // Validate client
  ...
  async handleLogin(email: string, password: string, params: LoginParams): Promise<string> {
    // Validate client
    const client = this.clientsStore.getClient(params.clientId);
    if (!client) {
      throw new Error('Invalid client');
  ...
  async handleDFCallback(
    ...
    const session = await res.json() as DFSession;
  ...
  async verifyAccessToken(token: string): Promise<DFAuthInfo> {
  ...
  private async authenticateDF(email: string, password: string): Promise<DFSession> {
    const headers: Record<string, string> = {
  ...
  private async validateDFSession(jwt: string): Promise<boolean> {
    try {
      const headers: Record<string, string> = {
